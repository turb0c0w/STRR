...
import { HeaderMenu, HeaderMenuItem } from '#components'
...
describe('Menu tests', () => {
  ...
  beforeEach(() => { wrapper = mount(HeaderMenu, { props: testProps }) })
  ...
  it('renders with required props', () => {
    ...
    expect(wrapper.findComponent(HeaderMenuItem).exists()).toBe(false)
  })
  ...
  it('opens the menu with expected items', async () => {
    ...
    for (let i = 0; i < testProps.menuLists.length; i++) {
      ...
      expect(menuList?.findAllComponents(HeaderMenuItem).length).toBe(testProps.menuLists[i].items.length)
    }
  })
})
