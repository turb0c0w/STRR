...
import { AlertsMessage } from '#components'
...
it('can mount AlertsMessage component', async () => {
  ...
  const warning = await mountSuspended(AlertsMessage,
  ...
  const message = await mountSuspended(AlertsMessage,
  ...
  const info = await mountSuspended(AlertsMessage,
